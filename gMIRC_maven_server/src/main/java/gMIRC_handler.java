 ...
 * @author Jonathan
 */
public class gMIRC_handler implements grpcMIRCGrpc.grpcMIRC {

  @Override
  public void join(gMIRC.UserChannel request,
	  io.grpc.stub.StreamObserver<gMIRC.ResCode> responseObserver) {
    int responseCode = AddChannel(request.getUsername(), request.getChannelname());
  ...
  @Override
  public void regUser(Username request, StreamObserver<ResCode> responseObserver) {
    //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    int responseCode = AddUser(request.getUsername());
    ResCode reply = ResCode.newBuilder().setResponseCode(responseCode).build();
    responseObserver.onValue(reply);
    responseObserver.onCompleted();
  }
  ...
  @Override
  public void message(UserMessage request, StreamObserver<ResCode> responseObserver) {
    //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    int ret = 0;
    if (request.getChannelname().equalsIgnoreCase("*")) {
      ret = PutMessageWild(request.getUsername(), request.getMsg());
    } else {
      ret = PutMessage(request.getUsername(), request.getChannelname(), request.getMsg());
    }
    ResCode reply = ResCode.newBuilder().setResponseCode(ret).build();
    responseObserver.onValue(reply);
    responseObserver.onCompleted();
  ...
  @Override
  public void regularUpdates(Username request, StreamObserver<UpdateString> responseObserver) {
    //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    String ret = GetMessage(request.getUsername());
    UpdateString reply = UpdateString.newBuilder().setNewMessages(ret).build();
    responseObserver.onValue(reply);
    responseObserver.onCompleted();
